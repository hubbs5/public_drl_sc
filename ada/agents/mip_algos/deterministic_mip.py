...
# scheduling environment on a shrinking horizon.
...
def buildDeterministicMIP(env, schedule=None, *args, **kwargs):
    # Build model
    m = ConcreteModel()
    ...
    m.fixed_horizon = m.sim_time + m.H
    ...
    # m.t = RangeSet(m.sim_time - m.delay - 1, m.K) # Schedule time intervals t
    m.t = RangeSet(m.sim_time - m.delay, m.sim_time + m.K) # Schedule time intervals t
    m.h = RangeSet(m.sim_time, m.sim_time + m.H) # Fixed time intervals h
    ...
    def shipment_constraint(m, n):
        return sum(m.x[i, t, n] for i in m.i for t in m.t if t >= m.sim_time) <= 1
